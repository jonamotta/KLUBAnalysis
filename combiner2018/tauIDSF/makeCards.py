...
for reg in regions:
	...
	if "both" in args.decay:
		if args.qcd:
			with open("combiner2018/tauIDSF/templates/datacard_template"+suffix+".txt","r") as template:
	...
	#### cross decay mode
		...
		print decay1, decay2
		if args.qcd:
			with open("combiner2018/tauIDSF/templates/datacard_template_cross"+suffix+".txt","r") as template:
				lines = template.readlines()
				with open(args.tag+"/"+args.ch+"/"+sel+"/datacard_QCD.txt", "w+") as outfile:
					for line in lines:
						line = line.replace('_NDATA'+suffix+'_',  str(round(hData.GetBinContent(0),2)))
...
if not args.qcd:
	...
	#os.chdir('../../..')
	os.chdir('/home/llr/cms/motta/CMSSW_10_2_16/src/KLUBAnalysis')
