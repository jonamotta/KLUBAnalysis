...
if __name__ == "__main__" :
	...
	parser.add_argument("--year", dest='year', help='year of the dataset', default='2018')
	...
	cfgName  =  args.dir + "/mainCfg_"+args.channel+"_Legacy{0}_binOptimization.cfg".format(args.year)
	#cfgName  =  args.dir + "/mainCfg_"+args.channel+"{0}_Legacy{1}_binOptimization.cfg".format(args.channel,args.year)
	...
	if "FlatSB" in args.tag:
		hBkgList.append(hSignal)
	...
	if "FlatSB" in args.tag: legmin = 0.8
	legminx = 0.50
	...
	if "FlatSB" in args.tag:
		if (len(hBkgNameList) +len(hSigs)> 4): leg.SetNColumns(2)
	leg.SetFillStyle(0)
	leg.SetBorderSize(0)
	leg.SetTextFont(43)
	leg.SetTextSize(20)
	...
	if "FlatSB" in args.tag: args.dosig = False
