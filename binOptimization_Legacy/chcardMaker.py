...
def parseOptions():
	...
	parser.add_option('--combination', dest='comb', type='string', default='no', help='are we doing multiple year combination? yes/no')
...
def  writeCard(input,theLambda,select,kLambda,region=-1):
	...
	if opt.comb == 'yes':
		outFile = "hh_{0}_C{1}_L{2}_13TeV_{3}.txt".format(thechannel,theCat,theLambda,opt.year)
	else:
	...
	if region < 0:
		...
		if opt.comb == 'yes':
			outroot = TFile.Open(out_dir+"hh_{0}_C{1}_L{2}_13TeV_{3}.input.root".format(thechannel,theCat,theLambda,opt.year),"RECREATE")
		else:
		...
		#print outFile
		if opt.comb == 'yes':
			outFile = "hh_{0}_C{1}_L{2}_13TeV_{3}_{4}.txt".format(thechannel,theCat,theLambda,regionName[region+1],opt.year)
		else:
...
for il in range(len(input.signals)):
	...
	input.signals[il] = input.signals[il].replace("VBFHHSM","VBFHH_bbtt11")
...
for (theLambda,kLambda) in zip(input.signals, kLambdas): # ---------> what happens when the signals and kLamndas are not the same length?? zip can't deal with different legths of arrays ###
	# kLambda refers to the actual value of the kLambda under consideration in this iteration (SM == 1)
	# theLambda refers to the 'index' of th kLambda value inside the kLambdas array (its just a position number, SM == 11)
