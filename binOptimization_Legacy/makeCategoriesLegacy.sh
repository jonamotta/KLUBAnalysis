#!/bin/bash
export TAG="optimized"
export CF="$CMSSW_BASE/src/KLUBAnalysis/binOptimization_Legacy/VBFoptimizedLimit_Legacy2017/"
export LEPTONS="TauTau ETau"
export SELECTIONS="VBFloose"
export YEAR="2017"
export VAR="DNNoutSM_kl_1"
...
do
		...
		do
				...
				python chcardMaker.py --filename ${CF}/analyzedOutPlotter_${c}.root --dir "_${TAG}" --channel ${c} --binbybin --selection ${BASE} ${RESONANT} --shape 1 --theory --path ${CF} --year ${YEAR} --var ${VAR} --lambda ${NAMESAMPLE}"11"
...
do
		...
		do
				...
				do
						...
						cd ${CF}/cards_${c}_${TAG}/${NAMESAMPLE}${i}${BASE}${arrVARIABLES[0]}
						pwd
						combineCards.py -S hh_*.txt &> comb.txt
						text2workspace.py comb.txt -m ${i} -o comb.root ;
						ln -ns /home/llr/cms/motta/CMSSW_10_2_16/src/KLUBAnalysis/binOptimization_Legacy/prepareHybrid.py . # the f option forces to overwrite the link if it already exists
...
do
		cd ${CF}
		mkdir -p cards_Combined_${TAG}/${NAMESAMPLE}${i}${arrVARIABLES[0]}
		pwd
		# MAKE COMBINATION PER CATEGORY [3 x mass point] AND CALCULATE COMBINED LIMITS
		for ibase in $SELECTIONS
		do
				mkdir -p cards_Combined_${TAG}/${NAMESAMPLE}${i}${ibase}${arrVARIABLES[0]}
				for c in $LEPTONS
				do
						export BASE="$ibase"
						...
						cp cards_${c}_${TAG}/${NAMESAMPLE}${i}${BASE}${arrVARIABLES[0]}/hh_*.* cards_Combined_${TAG}/${NAMESAMPLE}${i}${ibase}${arrVARIABLES[0]}/.
						cp cards_${c}_${TAG}/${NAMESAMPLE}${i}${BASE}${arrVARIABLES[0]}/hh_*.* cards_Combined_${TAG}/${NAMESAMPLE}${i}${arrVARIABLES[0]}/.
						cp cards_${c}_${TAG}/${NAMESAMPLE}${i}${BASE}${arrVARIABLES[0]}/hh_*.* cards_${c}_${TAG}/${NAMESAMPLE}${i}${arrVARIABLES[0]}/.
				done
				cd cards_Combined_${TAG}/${NAMESAMPLE}${i}${ibase}${arrVARIABLES[0]}
				pwd
		...
		# mkdir Impacts
		# cd Impacts
		# combineTool.py -M Impacts -d ../cards_Combined_optimized/ggHH_bbtt11BDToutSM_kl_1/comb.root -m ${i} --doInitialFit --robustFit 1
		# combineTool.py -M Impacts -d ../cards_Combined_optimized/ggHH_bbtt11BDToutSM_kl_1/comb.root -m ${i} --robustFit 1 --doFits --parallel 30
		# combineTool.py -M Impacts -d ../cards_Combined_optimized/ggHH_bbtt11BDToutSM_kl_1/comb.root -m ${i} --robustFit 1 -o impacts.json
