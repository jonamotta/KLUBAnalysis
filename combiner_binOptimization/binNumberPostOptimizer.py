...
def limit_extraction(nBins,new_bins_edges_2b0j,new_bins_edges_boosted,new_bins_edges_1b1j,new_bins_edges_VBFloose):
    ...
    file.close()
    prepend_line("makeCategories_postOpt.sh",'export SELECTIONS="s2b0jresolvedMcut sboostedLLMcut s1b1jresolvedMcut VBFlooseMcut"')
...
for column in df_flatS.columns.values:
    limit_extraction(int(column),df_flatS[column]["s2b0jresolvedMcut"],df_flatS[column]["sboostedLLMcut"],df_flatS[column]["s1b1jresolvedMcut"],df_flatS[column]["VBFlooseMcut"])
