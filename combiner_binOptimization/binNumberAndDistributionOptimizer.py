...
def HPlimit_extraction(nBins,new_bins_edges,mode_str,selection):
    ...
    mkdir_cmd = "mkdir NumbAndDistribScanSteps/{0}/{2}/bins_{1}".format(mode_str,str(nBins),sel)
    os.system(mkdir_cmd)
    ...
    copy_cmd = "cp -a {2}/mainCfg_{0} NumbAndDistribScanSteps/{3}/{4}/bins_{1}/; cp -a {2}/selectionCfg_{0} NumbAndDistribScanSteps/{3}/{4}/bins_{1}/; cp -a {2}/sampleCfg_Legacy{5}.cfg NumbAndDistribScanSteps/{3}/{4}/bins_{1}/; cp -a {2}/outPlotter.root NumbAndDistribScanSteps/{3}/{4}/bins_{1}/".format(tag,str(nBins),inDir,mode_str,sel,year)
    os.system(copy_cmd)

    # append rebinning section in the mainCfg
    mainCfg = open("NumbAndDistribScanSteps/{2}/{3}/bins_{0}/mainCfg_{1}".format(str(nBins),tag,mode_str,sel), "a")
    ...
    comb_cmd = "python ../scripts/combineFillerOutputs.py --dir NumbAndDistribScanSteps/{1}/{2}/bins_{0}".format(str(nBins),mode_str,sel)
    os.system(comb_cmd)

    # protection against almost empty bins -> can create bias in the final result
    RootToCheck = TFile.Open("NumbAndDistribScanSteps/{1}/{2}/bins_{0}/analyzedOutPlotter.root".format(str(nBins),mode_str,sel))
    ...
    wrapper_cmd = "python wrapperHistos.py -f NumbAndDistribScanSteps/{1}/{2}/bins_{0}/analyzedOutPlotter.root -c {3} -o bins_{0} -d NumbAndDistribScanSteps/{1}/{2}/bins_{0} -a 'GGF' -y {4}".format(str(nBins),mode_str,sel,channel,year)
    ...
    prepend_line("makeCategories_NandD.sh",'export CF="$CMSSW_BASE/src/KLUBAnalysis/combiner_binOptimization/NumbAndDistribScanSteps/{1}/{2}/bins_{0}/"'.format(str(nBins),mode_str,sel))
...
mkdir_cmd = "mkdir NumbAndDistribScanSteps"
os.system(mkdir_cmd)
cp_prog_cmd = "cp binNumberAndDistributionOptimizer.py NumbAndDistribScanSteps/"
os.system(cp_prog_cmd)

mkdir_cmd1 = "mkdir NumbAndDistribScanSteps/FlatS; mkdir NumbAndDistribScanSteps/FlatB; mkdir NumbAndDistribScanSteps/FlatSB; mkdir NumbAndDistribScanSteps/ConstSize"
os.system(mkdir_cmd1)
mkdir_cmd2 = "mkdir NumbAndDistribScanSteps/FlatS/2b0j; mkdir NumbAndDistribScanSteps/FlatB/2b0j; mkdir NumbAndDistribScanSteps/FlatSB/2b0j; mkdir NumbAndDistribScanSteps/ConstSize/2b0j"
os.system(mkdir_cmd2)
mkdir_cmd3 = "mkdir NumbAndDistribScanSteps/FlatS/1b1j; mkdir NumbAndDistribScanSteps/FlatB/1b1j; mkdir NumbAndDistribScanSteps/FlatSB/1b1j; mkdir NumbAndDistribScanSteps/ConstSize/1b1j"
os.system(mkdir_cmd3)
mkdir_cmd4 = "mkdir NumbAndDistribScanSteps/FlatS/boosted; mkdir NumbAndDistribScanSteps/FlatB/boosted; mkdir NumbAndDistribScanSteps/FlatSB/boosted; mkdir NumbAndDistribScanSteps/ConstSize/boosted"
os.system(mkdir_cmd4)
mkdir_cmd5 = "mkdir NumbAndDistribScanSteps/FlatS/VBFloose; mkdir NumbAndDistribScanSteps/FlatB/VBFloose; mkdir NumbAndDistribScanSteps/FlatSB/VBFloose; mkdir NumbAndDistribScanSteps/ConstSize/VBFloose"
os.system(mkdir_cmd5)
if year != "2016" and channel == "TauTau":
    mkdir_cmd6 = "mkdir NumbAndDistribScanSteps/FlatS/VBFtight; mkdir NumbAndDistribScanSteps/FlatB/VBFtight; mkdir NumbAndDistribScanSteps/FlatSB/VBFtight; mkdir NumbAndDistribScanSteps/ConstSize/VBFtight"
...
for column in df_results_2b0j.columns.values:
    ...
    #------
    df_results_VBFloose[column]['ConstSize'] = HPlimit_extraction(int(column),df_ConstSize[column]['VBFlooseMcut'],'ConstSize','VBFlooseMcut')*1000.
    df_results_VBFloose[column]['FlatS'] = HPlimit_extraction(int(column),df_flatS[column]['VBFlooseMcut'],'FlatS','VBFlooseMcut')*1000.
    df_results_VBFloose[column]['FlatB'] = HPlimit_extraction(int(column),df_flatB[column]['VBFlooseMcut'],'FlatB','VBFlooseMcut')*1000.
    df_results_VBFloose[column]['FlatSB'] = HPlimit_extraction(int(column),df_flatSB[column]['VBFlooseMcut'],'FlatSB','VBFlooseMcut')*1000.
    #------
    if year != "2016" and channel == "TauTau":
        df_results_VBFtight[column]['ConstSize'] = HPlimit_extraction(int(column),df_ConstSize[column]['VBFtightMcut'],'ConstSize','VBFtightMcut')*1000.
        df_results_VBFtight[column]['FlatS'] = HPlimit_extraction(int(column),df_flatS[column]['VBFtightMcut'],'FlatS','VBFtightMcut')*1000.
        df_results_VBFtight[column]['FlatB'] = HPlimit_extraction(int(column),df_flatB[column]['VBFtightMcut'],'FlatB','VBFtightMcut')*1000.
        df_results_VBFtight[column]['FlatSB'] = HPlimit_extraction(int(column),df_flatSB[column]['VBFtightMcut'],'FlatSB','VBFtightMcut')*1000.
...
for dist in ["ConstSize", "FlatB", "FlatS", "FlatSB"]:
    for bin in bins_window:
        df_results_2b0j[bin][dist] += get_result("NumbAndDistribScanSteps/{0}/2b0j/bins_{1}/cards_{2}_bins_{1}/ggHH_bbtt11s2b0jresolvedMcut{3}/out_Asym_ggHH_bbtt11_blind.log".format(dist,bin,channel,variable))*1000.
        df_results_boosted[bin][dist] += get_result("NumbAndDistribScanSteps/{0}/boosted/bins_{1}/cards_{2}_bins_{1}/ggHH_bbtt11sboostedLLMcut{3}/out_Asym_ggHH_bbtt11_blind.log".format(dist,bin,channel,variable))*1000.
        df_results_1b1j[bin][dist] += get_result("NumbAndDistribScanSteps/{0}/1b1j/bins_{1}/cards_{2}_bins_{1}/ggHH_bbtt11s1b1jresolvedMcut{3}/out_Asym_ggHH_bbtt11_blind.log".format(dist,bin,channel,variable))*1000.
        df_results_VBFloose[bin][dist] += get_result("NumbAndDistribScanSteps/{0}/VBFloose/bins_{1}/cards_{2}_bins_{1}/ggHH_bbtt11VBFlooseMcut{3}/out_Asym_ggHH_bbtt11_blind.log".format(dist,bin,channel,variable))*1000.
        if year != "2016" and channel == "TauTau":
            df_results_VBFtight[bin][dist] += get_result("NumbAndDistribScanSteps/{0}/VBFtight/bins_{1}/cards_{2}_bins_{1}/ggHH_bbtt11VBFtightMcut{3}/out_Asym_ggHH_bbtt11_blind.log".format(dist,bin,channel,variable))*1000.
...
# save the results
df_results_2b0j.to_html('NumbAndDistribScanSteps/NumbAndDistribScanResults_2b0j.html')
df_results_boosted.to_html('NumbAndDistribScanSteps/NumbAndDistribScanResults_boosted.html')
df_results_1b1j.to_html('NumbAndDistribScanSteps/NumbAndDistribScanResults_1b1j.html')
df_results_VBFloose.to_html('NumbAndDistribScanSteps/NumbAndDistribScanResults_VBFloose.html')
if year != "2016" and channel == "TauTau": df_results_VBFtight.to_html('NumbAndDistribScanSteps/NumbAndDistribScanResults_VBFtight.html')
...
# ok = True
# for column in df_flatSB.columns.values:
#    if len(df_flatSB[column]['VBFlooseMcut']) != int(column)+1:
#        print("-> "+str(len(df_flatSB[column]['VBFlooseMcut']))+" instead of "+str(int(column)+1))
#        ok = False
# if ok: print("-> all ok")
# print("VBFtightMcut")
# ok = True
# for column in df_flatSB.columns.values:
#    if len(df_flatSB[column]['VBFtightMcut']) != int(column)+1:
#        print("-> "+str(len(df_flatSB[column]['VBFtightMcut']))+" instead of "+str(int(column)+1))
#        ok = False
# if ok: print("-> all ok")
#
# print("\n----------------------------------")
...
# ok = True
# for column in df_flatB.columns.values:
#    if len(df_flatB[column]['VBFlooseMcut']) != int(column)+1:
#        print("-> "+str(len(df_flatB[column]['VBFlooseMcut']))+" instead of "+str(int(column)+1))
#        ok = False
# if ok: print("-> all ok")
# print("VBFtightMcut")
# ok = True
# for column in df_flatB.columns.values:
#    if len(df_flatB[column]['VBFtightMcut']) != int(column)+1:
#        print("-> "+str(len(df_flatB[column]['VBFtightMcut']))+" instead of "+str(int(column)+1))
#        ok = False
# if ok: print("-> all ok")
#
# print("\n----------------------------------")
...
# ok = True
# for column in df_ConstSize.columns.values:
#    if len(df_ConstSize[column]['VBFlooseMcut']) != int(column)+1:
#        print("-> "+str(len(df_ConstSize[column]['VBFlooseMcut']))+" instead of "+str(int(column)+1))
#        ok = False
# if ok: print("-> all ok")
# print("VBFtightMcut")
# ok = True
# for column in df_ConstSize.columns.values:
#    if len(df_ConstSize[column]['VBFtightMcut']) != int(column)+1:
#        print("-> "+str(len(df_ConstSize[column]['VBFtightMcut']))+" instead of "+str(int(column)+1))
#        ok = False
# if ok: print("-> all ok")
