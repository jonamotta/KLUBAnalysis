...
def HPlimit_extraction(nBins,new_bins_edges,mode_str,selection):
    ...
    file = open("makeCategories_NandD2018.sh".format(year),"r")
    lines = file.readlines()
    file.close()
    rm_cmd = "rm makeCategories_NandD2018.sh".format(year)
    os.system(rm_cmd)
    file = open("makeCategories_NandD2018.sh".format(year),"w")
    file.writelines(lines[7:])
    file.close()
    prepend_line("makeCategories_NandD2018.sh".format(year),'export VAR="{0}"'.format(variable))
    prepend_line("makeCategories_NandD2018.sh".format(year),'export YEAR="{0}"'.format(year))
    prepend_line("makeCategories_NandD2018.sh".format(year),'export SELECTIONS="{0}"'.format(selection))
    prepend_line("makeCategories_NandD2018.sh".format(year),'export LEPTONS="{0}"'.format(channel))
    prepend_line("makeCategories_NandD2018.sh".format(year),'export CF="$CMSSW_BASE/src/KLUBAnalysis/combiner_binOptimization/NumbAndDistribPostOpt/{1}/{2}/bins_{0}/"'.format(str(nBins),mode_str,sel))
    prepend_line("makeCategories_NandD2018.sh".format(year),'export TAG="bins_{0}"'.format(str(nBins)))
    prepend_line("makeCategories_NandD2018.sh".format(year),"#!/bin/bash")

    # call makeCategories_NandD.sh
    limits_cmd = "./makeCategories_NandD2018.sh".format(year)
    os.system('chmod 777 makeCategories_NandD2018.sh'.format(year))
...
if variable == "BDToutSM_kl_1":
    NandD2018 = [
                    [
                        ["FlatS", "FlatB", "FlatS", "FlatS"], ["FlatS", "FlatB", "FlatS", "FlatS"], ["FlatB", "FlatB", "FlatSB", "FlatB", "FlatB"]  # line of the distributions: [ [Etau], [MuTau], [TauTau] ] -> [2b0j,boost,1b1j,VBFloose] each
                    ],
                    [
                        [18, 14, 28, 10], [18, 26, 30, 14], [50, 10, 48, 48, 36]  # line of the number of bins: [ [Etau], [MuTau], [TauTau] ] -> [2b0j,boost,1b1j,VBFloose] each
                    ],
                    [
                        [df_flatS, df_flatB, df_flatS, df_flatS], [df_flatS, df_flatB, df_flatS, df_flatS], [df_flatB, df_flatB, df_flatSB, df_flatB, df_flatB] # line of the binnings: [ [Etau], [MuTau], [TauTau] ] -> [2b0j,boost,1b1j,VBFloose] each
                    ]
                ]
    NandD2016 = [
                    [
                        ["FlatS", "FlatB", "FlatS", "FlatSB"], ["FlatS", "FlatB", "FlatS", "FlatS"], ["FlatB", "ConstSize", "FlatSB", "FlatB"]  # line of the distributions: [ [Etau], [MuTau], [TauTau] ] -> [2b0j,boost,1b1j,VBFloose] each
                    ],
                    [
                        [16, 12, 26, 50], [18, 16, 26, 12], [42, 10, 36, 30]  # line of the number of bins: [ [Etau], [MuTau], [TauTau] ] -> [2b0j,boost,1b1j,VBFloose] each
                    ],
                    [
                        [df_flatS, df_flatB, df_flatS, df_flatSB], [df_flatS, df_flatB, df_flatS, df_flatS], [df_flatB, df_ConstSize, df_flatSB, df_flatB]  # line of the binnings: [ [Etau], [MuTau], [TauTau] ] -> [2b0j,boost,1b1j,VBFloose] each
                    ]
                ]
if variable == "DNNoutSM_kl_1":
    ...
    NandD2016 = [
                    [
                        ["FlatSB", "FlatB", "FlatS", "FlatSB"], ["FlatSB", "FlatB", "FlatS", "FlatSB"], ["ConstSize", "ConstSize", "FlatS", "FlatB"]  # line of the distributions: [ [Etau], [MuTau], [TauTau] ] -> [2b0j,boost,1b1j,VBFloose] each
                    ],
                    [
                        [48, 12, 12, 22], [50, 10, 10, 28], [36, 10, 12, 36]  # line of the number of bins: [ [Etau], [MuTau], [TauTau] ] -> [2b0j,boost,1b1j,VBFloose] each
                    ],
                    [
                        [df_flatSB, df_flatB, df_flatS, df_flatSB], [df_flatSB, df_flatB, df_flatS, df_flatSB], [df_ConstSize, df_ConstSize, df_flatS, df_flatB]  # line of the binnings: [ [Etau], [MuTau], [TauTau] ] -> [2b0j,boost,1b1j,VBFloose] each
                    ]
                ]
