...
if __name__ == "__main__":
	...
	# RUN COMBINE FOR A BLIND FIT
	scriptFile.write('combine -M AsymptoticLimits %s/comb.root -m %d -n %s_forLim --run blind &> out_Asym_%s_blind.log \n' % (jobsDir,opt.m,opt.n,opt.n))
	scriptFile.write('combine -M AsymptoticLimits %s/comb.root -m %d -n %s_forLim_noTH --freezeNuisanceGroups theory --run blind &> out_Asym_%s_noTH_blind.log \n' % (jobsDir,opt.m,opt.n,opt.n))

	# RUN COMBINE FOR AN UN-BLIND FIT
	...
	os.system("/home/llr/cms/motta/t3submit -short \'%s/runJob_Asym_%s.sh\'"%(jobsDir,opt.n))
