#!/bin/bash
export TAG="Combined"
export CF="$CMSSW_BASE/src/KLUBAnalysis/combiner_binOptimization/CombinedBenchmark_DNN/"
export LEPTONS="ETau MuTau TauTau "
export SELECTIONS="s2b0jresolvedMcut sboostedLLMcut s1b1jresolvedMcut VBFlooseMcut VBFtightMcut"
export YEARS="2016 2018"
export VARIABLE="DNNoutSM_kl_1"

# ggHH_bbtt IS THE OLD NAME FOR SM PRODUCTION OF THE HH PAIR
export NAMESAMPLE="ggHH_bbtt11"
export RESONANT=$2

echo $VARIABLE
echo $YEARS
echo $LEPTONS
echo $SELECTIONS
echo ""

export QUANTILES="0.025 0.16 0.5 0.84 0.975 -1.0"


# CREATE ALL THE CARDS -> SELECTIONS x CHANNELS (AT MOST 13 CASES = 3 CHANNELS x 5 SELECTIONS - 2 NON EXISTING COMBINATIONS)
for s in $SELECTIONS
do
		for c in $LEPTONS
		do
				for y in $YEARS
				do
						# VBFtight SELECTION DOES NOT EXIST FOR THE MuTau/ETau CHANNEL AND FOR THE 2016
						if [ "${y}" == "2016" ]
						then
								if [[ "${s}" == *"VBFtight"*  ]]
								then
										continue
								fi
						fi
						if [ "${c}" == "MuTau" ]
						then
								if [[ "${s}" == *"VBFtight"*  ]]
								then
										continue
								fi
						fi
						if [ "${c}" == "ETau" ]
						then
								if [[ "${s}" == *"VBFtight"*  ]]
								then
										continue
								fi
						fi
						python chcardMaker.py -f ${CF}analyzedOutPlotter_${c}_${y}.root -o "_${y}" -c ${c} -y -s ${s} ${RESONANT} -u 1 -t -p ${CF} --year ${y} --var ${VARIABLE} --combination "yes"
				done
		done
done

echo ""
echo "ALL CARDS CREATED"
echo "STARTING COMBINATION AND SINGLE LIMIT CALCULATION"
echo ""

mkdir ${CF}/cards_${TAG}_${VARIABLE}
for s in $SELECTIONS
do
		for c in $LEPTONS
		do
				for y in $YEARS
				do
						if [ "${y}" == "2016" ]
						then
								if [[ "${s}" == *"VBFtight"*  ]]
								then
										continue
								fi
						fi
						if [ "${c}" == "MuTau" ]
						then
								if [[ "${s}" == *"VBFtight"*  ]]
								then
										continue
								fi
						fi
						if [ "${c}" == "ETau" ]
						then
								if [[ "${s}" == *"VBFtight"*  ]]
								then
										continue
								fi
						fi
						cp ${CF}/cards_${c}_${y}/${NAMESAMPLE}${s}${VARIABLE}/* ${CF}/cards_${TAG}_${VARIABLE}/
				done
		done
done

cd ${CF}/cards_${TAG}_${VARIABLE}
combineCards.py -S hh_*_C1_L${NAMESAMPLE}_13Te*.txt hh_*_C2_L${NAMESAMPLE}_13Te*.txt hh_*_C3_L${NAMESAMPLE}_13Te*.txt hh_*_C4_L${NAMESAMPLE}_13Te*.txt hh_*_C5_L${NAMESAMPLE}_13Te*.txt >> comb.txt
text2workspace.py -m "11" comb.txt -o comb.root ;
ln -ns /home/llr/cms/motta/CMSSW_10_2_16/src/KLUBAnalysis/combiner_binOptimization/prepareHybrid.py .
ln -ns /home/llr/cms/motta/CMSSW_10_2_16/src/KLUBAnalysis/combiner_binOptimization/prepareGOF.py .
ln -ns /home/llr/cms/motta/CMSSW_10_2_16/src/KLUBAnalysis/combiner_binOptimization/prepareAsymptotic.py .
ln -ns /home/llr/cms/motta/CMSSW_10_2_16/src/KLUBAnalysis/combiner_binOptimization/prepareImpacts.py .
python prepareAsymptotic.py -m "11" -n ${NAMESAMPLE}
